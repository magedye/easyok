...
The file `.env.example` (derived from this contract) is the **single source of truth**
...
**Forbidden in reference templates:**
...
- Read-only DB access is mandatory
...
Feature toggles may be modified **only via Admin API** when enabled:
...
Only one DB provider is active:
...
- Disabled ≠ Silent
- No-Op services MUST emit telemetry
